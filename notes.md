...
## 2. 確率と確率分布
  ...
  |二項分布|$ P(X=x) = f(x) \space = _nC_xp^x(1-p)^{n-x}$<p><p>期待値<br>$E[X] = np \\ = \sum_{x=0}^{n}x \frac{n!}{x!(n-x)!} p^x(1-p)^{n-x}$<p><p>分散<br>$V[X] = np(1-p) \\ = E[X(X-1)] + E[X]-(E[X])^2 \\ = n(n-1)p^2+np-(np)^2$|成功率$p$の$n$回ベルヌーイ試行を行ったとき、<br>成功回数が$x$となる確率<p>$B(n,p)$と記すこともある||
