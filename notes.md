...
## 1. データの記述と要約
  ...
  |||||
  |||||
  |相関係数|$r_{xy} = \frac{S_{xy}}{S_x S_y}$<p>$S_{xy}$は共分散<br>$S_{xy} = \frac{1}{n} \sum_{i=1}^{n} (x_i - \bar{x})(y_i - \bar{y})$<p>$S_x$は$x$の標準偏差<p>$S_y$は$y$の標準偏差|データ$x$と$y$の相関をみることができる<p>必ず$-1 \leqq r_{xy} \leqq 1$の値を取る||
  |偽相関||見かけ上の相関、関係性がないのに相関があるように見えること||
  |偏相関係数|$r_{(yz.x)} = \frac{r_{yz} - r_{xy} r_{xz}}{\sqrt{1 - r_{xy}^2} \sqrt{1 - r_{xy}^2}}$|偽相関を見抜く方法<p>データ$x$、$y$、$z$があったときに$x$による相関の影響を除く||
  ...
  |||||
  |||||
  |ラスパイレス指数|$P_L = \frac{\sum_{i=1}^{n} p_{it} q_{0i}}{\sum_{i=1}^{n} p_{0i} q_{0i}} \times 100$<p>$p_{0i}$：基準年の価格<br>$q_{0i}$：基準年の数量<br>$p_{ti}$：比較年の価格<br>$q_{ti}$：比較年の数量|基準年の購入量や取引量を重みとして算出する価格指数<p>基準年の値を基準として計算して比較する<p>基準年と比べてどれだけ、変化したかを表す||
  |パーシェ指数|$P_P = \frac{\sum_{i=1}^{n} p_{it} q_{ti}}{\sum_{i=1}^{n} p_{0i} q_{ti}} \times 100$<p>$p_{0i}$：基準年の価格<br>$q_{0i}$：基準年の数量<br>$p_{ti}$：比較年の価格<br>$q_{ti}$：比較年の数量|比較年の購入量や取引量を重みとして算出する価格指数<p>比較年の値を基準として計算して比較する<p>基準年の価格で同じものを購入した場合の費用を計算||
  |フィッシャー指数|$\sqrt{P_L \times P_P}$|ラスパイレス指数とパーシェ指数の幾何平均によって算出<p>理想的な指数とされているが計算が複雑||
  |||||
  |||||
...
## 2. 確率と確率分布
  ...
  |累積分布関数|離散型<br>$F(x) = P(X \leq x) = \sum_{X \leq x} P(X)$<p><p>連続型<br>$F(x) = P(-\infty \leqq X \leqq x) \\ = \int_{-\infty}^{x}f(t)dt$|確率変数$X$が**ある値$x$以下$(X \leq x)$の値となる確率**<p>大文字のFを用いて$F(x)$と表わす<p>離散型<br><img src="image-12.png" width="300px"><p><p>連続型<br><img src="image-13.png" width="300px"><p><p>確率密度関数$f$ <--> 累積分布関数$F$と<br>表わされる<p><p>$F(\infty) = 1$<br>確率変数がある範囲の値しかとらないときに、$X$がとる値がその範囲の最大値以上となる場合に成立する<p><p>$F(-\infty)$<br>確率変数$X$がとる値がマイナス無限大となる確率は0である||
  |期待値|離散型<br>$E[X] = \sum_i x_if(x_i) = \mu$<p>連続型<br>$\int_{-\infty}^{\infty}x(fx)dx = \mu$|平均と同じ意味||
  |分散|離散型<br>$V[X] = E[(X-\mu)^2] \space = \sum_i(x_i - \mu)^2f(x_i) \space = \sigma^2$<p>連続型<br>$V[X] = E[(X-\mu)^2]$<br>$= \int_{-\infty}^{\infty}(x-\mu)^2f(x)dx \space = \mu^2$<p>また。。。<br>$V[X] = E[X^2] - \mu^2$<br>としても定義される|バラつきを表す指標となり標準偏差（$\sigma$）の2乗||
  ...
  |二項分布|$ P(X=x) = f(x) \space =  _nC_xp^x(1-p)^{n-x}$<p><p>期待値<br>$E[X]=np \space = \sum_{x=0}^{n}x \frac{n!}{x!(n-x)!} p^x(1-p)^{n-x}$<p><p>分散<br>$V[X] = np(1-p) \\ =E[X(X-1)]+E[X]-(E[X])^2 \\ = n(n-1)p^2+np-(np)^2$|成功率$p$の$n$回ベルヌーイ試行を行ったとき、<br>成功回数が$x$となる確率<p>$B(n,p)$と記すこともある||
